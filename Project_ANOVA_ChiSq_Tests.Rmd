...
AllModel<-glm(formula = FATALITIES_COUNT ~ EVENT_TYPE +STATE +MONTH_NAME+EVENT_TYPE+FATALITY_LOCATION+POP+PERFAT, family = poisson(link=log), data = storm)

summary(AllModel)
summary(AllModel)$coefficients
pR2(AllModel)
anova(AllModel, test="Chisq")
